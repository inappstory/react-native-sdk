...
// TODO async client https://github.com/Microsoft/typed-rest-client/blob/master/lib/RestClient.ts
        ...
        return new Promise((resolve, reject) => {
            ...
            api.get(resource, {params})
                .then(function (response) {
                    ...
                    let models: Array<A> = [];
                    (val as Array<IModelValues>).forEach((values: IModelValues) => {
     ...
     * @param method
     */
    public static update(resource: string, payLoad: Dict, method: Method = 'post') {
    ...
    public static sendBeacon(path: string, data: Dict, cb: () => void): void {
        if ("sendBeacon" in navigator) {

            const url = `${trimEndSlash(api.defaults.baseURL)}/${trimStartSlash(path)}?access_token=${String((api.defaults.headers as any)?.Authorization).replace(/^Bearer /, "")}&session_id=${getSessionId()}`;
