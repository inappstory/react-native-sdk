...
export default class StoriesItem extends AbstractModel {
  ...
  public slides_duration: Array<number> = [];
    public slides_screenshot_share: Array<number> = [];
  ...
  public static fetchList(placeholders: Dict<any>, tags: Option<string>, favorite?: boolean): Promise<IDataProvider<StoriesItem>> {
    this.placeholders = placeholders;
  ...
  get isOpened(): boolean {
    ...
    //     let data: Option<Array<number>> = localStorageGet(key);
    //     if (!isArray(data)) {
    //         data = [];
    //     }
