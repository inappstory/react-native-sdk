...
export default function rotatingSlider(selector, options) {
  ...
  function initSingleSlider($el, options) {
    ...
    let __opts = $.extend(defaultOptions, options);

    let axis      = (__opts.xRotation) ? "Y" : "X";
    let angleMult = (__opts.xRotation) ? 1 : -1;
    ...
    function rotateSlider(delta) {
      ...
      setTimeout(function() {
        ...
        if (delta > 0) {
          $rotaters.trigger('finish:rotaters:next')

        } else {
          $rotaters.trigger('finis:rotaters:prev')
        }
    ...
    function initSlider($el) {
      ...
      $rotaters.on('rotaters:next', () => {if (rotating && !__opts.allowControlsDuringAnim) return; navigateDown()})
      $rotaters.on('rotaters:prev', () => {if (rotating && !__opts.allowControlsDuringAnim) return; navigateUp()})
