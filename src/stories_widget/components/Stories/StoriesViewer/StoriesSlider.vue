<template>
  <div class="stories-item" ref="storiesItem">
    ...
    <stories-slide-item :count="slidesCount" :hiddenTimeline="hiddenTimeline" v-for="(slide, index) in displayedItems" :key="index"
                        :item="slide"
                        :index="index"
...
const StoriesSlider = Vue.extend({
  ...
  computed: {
    ...
    slidesCount: function (): number | null {
      ...
      hiddenTimeline: function (): boolean {
          if (this.item === undefined) {
              return true;
          }

          if (Array.isArray(this.item.slides_duration) && this.item.slides_duration.length === 1 && this.item.slides_duration[0] === 0) {
              // hide ticks if one slide with disabled timeline
              return true;
          }
