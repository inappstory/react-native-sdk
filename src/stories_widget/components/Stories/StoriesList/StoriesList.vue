<template>
  <StoryCardsWrapper class="stories-list-wrapper"
                     :class="{loaded, 'mode-desktop': modeDesktop, 'with-list-controls': withListControls}"
                     :style="rootVars">
    <div class="stories-list">
      ...
      <div class="stories-list__viewport" v-if="displayedItems.length > 0" :style="{
        ...
        <div class="stories-list__viewport-inner mode-desktop" :class="{'with-list-controls': withListControls, ...sliderAlignClass, isTitleCardOutsideTop, isTitleCardOutsideBottom}"
             ref="viewportInner" v-if="modeDesktop" v-dragscroll.x="{positionX:scrollPositionX}"
             v-on:dragscrollstart="scrollStart" v-on:dragscrollend="scrollEnd($event.detail)" :style="{height: `${sliderOptions.layout.storiesListInnerHeight}px`}">
        ...
        <div class="stories-list__viewport-inner" :class="{...sliderAlignClass, isTitleCardOutsideTop, isTitleCardOutsideBottom}" ref="viewportInner" :style="{height: `${sliderOptions.layout.storiesListInnerHeight}px`}" v-else>
          <StoryCardItem v-for="(item, index) in displayedItems" :key="item.id" :item="item" :index="index"
                         :data-id="item.id"
...
import StoryCardItem from "../../Stories/StoryCardItem.vue";
import StoryCardsWrapper from "../../Stories/StoryCardsWrapper.vue";
  ...
  computed: {
    ...
    ...mapGetters('shared', { // связать с данными хранилища
      ...
      storiesBackgroundColor: 'storiesBackgroundColor',
        storiesListSliderAlign: 'storiesListSliderAlign',
      ...
      storiesListItemFavoriteOptions: 'storiesListItemFavoriteOptions',
        ...
        storiesListItemOptions: 'storiesListItemOptions',
    ...
    favoriteCell: function (): Option<StoriesItem> {
    ...
    sliderAlignClass(): Dict<boolean> {
        return {
            "align-left": this.storiesListSliderAlign === StoriesListSliderAlign.LEFT,
            "align-center": this.storiesListSliderAlign === StoriesListSliderAlign.CENTER,
            "align-right": this.storiesListSliderAlign === StoriesListSliderAlign.RIGHT
        };
    },
    isTitleCardInner(): boolean {
        return this.storiesListItemOptions.title.position === StoriesListCardTitlePosition.CARD_INSIDE_BOTTOM;
    },
    isTitleCardOutsideTop(): boolean {
        return this.storiesListItemOptions.title.position === StoriesListCardTitlePosition.CARD_OUTSIDE_TOP;
    },
    isTitleCardOutsideBottom(): boolean {
      ...
      this.$nextTick(() => {
        if (this.$refs.viewportInner) {
          if (!this.thumbViewsHandlerInited) {
              if (this.$refs.viewportInner.children.length) {
