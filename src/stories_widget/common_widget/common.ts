...
(window as any).parseJSON = ((window as any).JSON && JSON.parse) ? function (obj: any) {
...
(window as any).cur = {destroy: [], nav: []}; // Current page variables and navigation map.
...
(window as any)._logTimer = (new Date()).getTime();
export function debugLog(msg: any) {
    try {
        const t = '[' + (((new Date()).getTime() - (window as any)._logTimer) / 1000) + '] ';
/*        if (ge('debuglog')) {
            if (msg === null) {
                msg = '[NULL]';
...
export function loadScript(scriptSrc: string, options: any) {
    ...
    function success(evt: any) {
        if ((script as any).readyState && (script as any).readyState != 'loaded' && (script as any).readyState != 'complete') return;
