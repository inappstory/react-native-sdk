...
export default {
  ...
  UPDATE_STORIES_READED_AND_STAT: (state, {readedIds, dataLength}) => {
    ...
    readedIds.forEach(id => {
      ...
      state.stories[id].setIsOpened(true);
    })
  ...
  UPDATE_STORIES_ORDER: (state) => {
    ...
    storiesList.sort(async (a, b) => {
      if (await a.getIsOpened() === true && await b.getIsOpened() === false) {
        return 1
      } else if (await a.getIsOpened() === false && await b.getIsOpened() === true) {
        return -1
      } else {
