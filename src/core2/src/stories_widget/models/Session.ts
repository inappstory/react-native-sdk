...
import ImageSet from "~/src/stories_widget/models/ImageSet";

import {localStorageGet, localStorageSet} from "~/src/stories_widget/helpers/localStorage"
...
export async function ensureSession(): Promise<Nullable<Session>> {
    if (storeLink !== null) {
        debug(storeLink.getters['shared/session'])
        let session: Session = storeLink.getters['shared/session'];
        debug(session)
...
export class Session extends AbstractModel {
    ...
    private _projectResourceCache!: Nullable<ProjectResourceCacheSet>;
    ...
    public attributes(): Array<string> {
        return concat(super.attributes(), ['server_timestamp', 'session', 'cache', 'share', 'user_key', 'placeholders', 'is_allow_profiling']);
    }
    ...
    private static getDeviceId(): Promise<ISessionInitData> {
        ...
        return new Promise<ISessionInitData>((resolve, reject) => {
            let device_data: Nullable<ISessionInitData> = localStorageGet('device_data');
            let device_id = null;
            if (device_data !== null) {
    ...
    public static updateSession(data: IData) {
        return this.update('session/update', data);
    }

    public static closeSession(data: IData) {
        let session: Session = storeLink.getters['shared/session'];
        if (!session.closed) {
            session.closed = true;
    ...
    public static get userKey(): Nullable<string>
    {
        let session: Session = storeLink?.getters['shared/session'];
        if (session) {
    ...
    public get cache(): Nullable<ProjectResourceCacheSet> | any {
