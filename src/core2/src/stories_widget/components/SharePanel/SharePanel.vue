            ...
            <transition :appear="true" name="swipe-up" @after-leave="closePanel">
                ...
                <div class="share-panel-row title-row">
                    <div class="share-url-favicon"><img
                        :src="`https://www.google.com/s2/favicons?sz=64&domain=${shareUrlDomain}`" alt=""></div>
...
import FacebookIcon from "~/src/stories_widget/components/svg/FacebookIcon.vue";
import TwitterIcon from "~/src/stories_widget/components/svg/TwitterIcon.vue";
import LinkedInIcon from "~/src/stories_widget/components/svg/LinkedInIcon.vue";
import VkIcon from "~/src/stories_widget/components/svg/VkIcon.vue";
import CopyIcon from "~/src/stories_widget/components/svg/CopyIcon.vue";
import {addOnceEventListener} from "~/src/stories_widget/util/event";

const Clipboard = require('~/src/stories_widget/helpers/clipboard');
...
export default class SharePanel extends Vue {
    ...
    updatePanelHeight() {
        if (this.$refs.sharePanel) {
            this.panelHeight = window.getComputedStyle((<any>this.$refs.sharePanel)).height;
        }
    }
