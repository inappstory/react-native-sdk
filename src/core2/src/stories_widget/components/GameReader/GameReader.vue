...
export default class GameReader extends Vue {
    ...
    @Prop({type: Object as PropType<Nullable<GameData>>}) gameData!: Nullable<GameData>;
    @Prop({type: Boolean}) desktopMode!: boolean;

    isLoading: boolean = true;
    isGameInited: boolean = false;
    iframeElement!: HTMLIFrameElement;
    shareLink: Nullable<string> = null;
    sharePanelOpen = false;

    get coverFile(): Nullable<string> {
    ...
    get gameFileOrigin(): Nullable<string> {
                ...
                for (let key of Object.keys(config)) {
                    if (config.hasOwnProperty(key)) {
                        for (let i = 0; i < resources.length; i++) {
                            if (resources[i]['key'] === (<any>config)[key]) {
                                (<any>config)[key] = resources[i]['url'];
                            }
                        }
                    }
                }
    ...
    get gameReaderWindow(): Nullable<Window> {
    ...
    updateIframeContent(content: string): void {
        if (this.$refs['gameReaderIframe'] !== null && this.$refs['gameReaderIframe'] !== undefined) {
            const iframeElement = (<HTMLFrameElement>this.$refs['gameReaderIframe']);
    ...
    _closeGameReader(data?: object): void {
      ...
      // @ts-ignore
        (<StoriesViewer>this.$parent).closeGameReader();
      // @ts-ignore
        (<StoriesViewer>this.$parent).resumeAndEnableReaderUIForGameReader();
        this.isGameInited = false;

        // pass ev to story
        if ((<any>window)._narrative_game) {
            (<any>window)._narrative_game.complete(data);
        }
