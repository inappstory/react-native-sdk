...
<script lang="ts">
    ...
    import {_winWidth, _winHeight} from '~/src/stories_widget/util/window-size';
    import {IAnimateOptions, animate} from '~/src/stories_widget/util/animation';
    import {WidgetStoriesOptions} from "~/src/stories_widget/models/WidgetStoriesOptions";

    import Article, {ArticleOptions} from "~/src/stories_widget/models/Article";
    import IssueArticle, {IssueArticleOptions} from "~/src/stories_widget/models/IssueArticle"
    import StoriesViewer from '~/src/stories_widget/components/Stories/StoriesViewer/StoriesViewer.vue';
    ...
    export default class ArticleViewer extends Vue {
        ...
        iframeOnLoad(el: Event): void {
            const iframeElement = (<HTMLFrameElement>el.target);
            ...
            if (this.type === IssueArticleOptions.TYPE) {
                this.$store.dispatch(`${namespace}/FETCH_ISSUE_ARTICLES`, [this.id]).then(_ => {
                    ...
                    if (issueArticle !== null) {
                        ...
                        this.headerTitle = (<IssueArticle>issueArticle).magazine_name;

                        this.updateIframeContent((<IssueArticle>issueArticle).content);
                ...
                this.$store.dispatch(`${namespace}/FETCH_ARTICLES`, [this.id]).then(_ => {
                    ...
                    if (article !== null) {
                        this.headerTitle = (<Article>article).magazine_name;
                        this.updateIframeContent((<Article>article).content);
                    }
        ...
        updateIframeContent(content: string): void {
            if (this.$refs['articleIframe'] !== null && this.$refs['articleIframe'] !== undefined) {
                const iframeElement = (<HTMLFrameElement>this.$refs['articleIframe']);
                if (iframeElement.contentWindow !== null && iframeElement.contentWindow.document !== null) {
        ...
        closeArticleViewer(): void {
          ...
          // @ts-ignore
            (<StoriesViewer>this.$parent).closeArticleViewer();

          // @ts-ignore
            (<StoriesViewer>this.$parent).resumeAndEnableViewerUIForArticleViewer();
        }
