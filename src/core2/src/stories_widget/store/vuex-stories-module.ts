...
import {StoryFavoriteReaderOptions, StoryFavoriteReaderOptionsDefault} from "~/src/types/storyManager/storyFavoriteReader";
import {StoryReaderOptions, StoryReaderOptionsDefault} from "~/src/types/storyManager/storyReader";
import {AppearanceCommonOptions, AppearanceCommonOptionsDefault} from "~/src/types/storyManager/appearanceCommon";
...
export interface StoriesState {
  ...
  muted: boolean;
  nextFlippingStoryId: Nullable<number>;
  activeListType: STORY_LIST_TYPE;
  sliderWidth: Nullable<number>;
...
const module: VuexModule<StoriesState, RootState> = {
  ...
  getters: {
    ...
    firstActiveStoryId: (state: StoriesState, getters: GetterTree<StoriesState, RootState>): number | null => {
      const value = first(state.storiesIds);
      if (isNumber(value)) {
        return value;
      }
      return null;
    ...
    nextFlippingStoryId: (state: StoriesState): Nullable<number> => state.nextFlippingStoryId,

    sliderWidth: (state: StoriesState): Nullable<number> => state.sliderWidth,
  ...
  mutations: {
    ...
    setFavoriteOptions: (state: StoriesState, options: StoryFavoriteReaderOptions) => {
      state.favoriteOptions = merge(state.favoriteOptions, options);
    },
    ...
    setNextFlippingStory: (state: StoriesState, value: Nullable<number>) => {
