...
import { domains } from "../story-manager/api/domains";
...
class WidgetStoryReader extends Widget implements IWidgetStoryReader {
    ...
    private openStoryReaderWithOptions: Dict = {};

    constructor(
        private appearanceManagerGetter: () => Option<AppearanceManager>,
        private overrideStoryReaderOptions: Pick<
            StoryReaderOptions,
            "recycleStoriesList" | "closeOnLastSlideByTimer"
        > = {}
    ) {
        super();
        ...
        RPC.onReaderOpened.use((options) => {
            ...
            this.emit("openedStoryReader", options);
        ...
        // show share panel
        // todo - React - move SharePanel from StoryViewer
        RPC.onShareStoryInternal.use(async (shareData) => {
            let success = false;
            let options: Dict = {};
            RPC.onPauseUI();
            if (shareData.url) {
            ...
            } else if (shareData.files && Array.isArray(shareData.files)) {
                options = {
                    urls: shareData.files,
                };
            }
            try {
                const result = await Share.open(options);
                if (isObject(result) && isBoolean(result.success)) {
        ...
        const backPressHandler = () => {
            const isClosed = this.closeStoryReaderOrLoader();
            // return true back btn handled, break event bubble
            // else - use another handler
            return isClosed;
        };
        ...
        this.on("openStoryReader", (options: { windowReferer: STORY_READER_WINDOW_REFERER }) => {
            this.modalPresentation.isOpen = true;
            this.openStoryReaderWithOptions = options;

            BackHandler.addEventListener("hardwareBackPress", backPressHandler);
        });

        // widget opened and connected to view
        this.on("openedStoryReader", (options: { windowReferer: STORY_READER_WINDOW_REFERER }) => {
            this.modalPresentation.isWidgetConnected = true;
        });

        // connected widget closed
        // this.on('closeStoryReader', (options: { windowReferer: STORY_READER_WINDOW_REFERER }) => {
    ...
    public closeStoryReaderOrLoader() {
        if (this.modalPresentation.isWidgetConnected) {
            ...
            this.emit("closeStoryReader", this.openStoryReaderWithOptions);
    ...
    private get storyReaderOptions(): StoryReaderOptions & { appearanceCommon: AppearanceCommonOptions } {
        ...
        if (appearanceManager != null) {
            return {
                ...
                faviconApiUrl: domains.faviconApiUrl,
            };
        } else {
            return {
    ...
    public async mount(
        ...
        storiesListOptions: StoriesListOptions & Pick<AppearanceCommonOptions, "hasFavorite">
    ): Promise<boolean> {
        // создаем iframe
        ...
        if (storiesListOptions.layout.height) {
            ...
            if (!storiesListOptions.title) {
                options.startHeight =
                    storiesListOptions.card.height +
                    storiesListOptions.topPadding +
                    storiesListOptions.bottomPadding +
                    storiesListOptions.bottomMargin;
            } else {
    ...
    public get containerOptions(): Dict {
        ...
        if (params.storiesLayoutHeight) {
            ...
            if (!params.storiesTitle) {
                defaults.startHeight =
                    parseFloat(params.storiesHeight || defaults.storiesHeight) +
                    parseFloat(params.storiesListTopPadding) +
                    parseFloat(params.storiesListBottomPadding) +
                    parseFloat(params.storiesListBottomMargin);
            } else {
                // line height + margin-bottom
                defaults.startHeight =
                    50 +
        ...
        const cssWidth: string = width
            ? Math.max(defaults.minWidth || 200, Math.min(defaults.maxWidth || 10000, width)) + "px"
            : "100%";
     ...
     */
        ...
        return {
            ...
            source: {
                html: this.widgetEntrypoint,
                baseUrl: "https://sdk.inappstory.com",
            },
    ...
    public async init(
        ...
        return this._init(
            mountSelector as ObjectId,
            options,
            params,
            storiesListOptions,
            functions,
            defaults,
            function (o: any, i: any, r: any) {
                widgetRoot = o;
                widget = i;
                rpc = r;
            },
...
`;
    ...
    protected async _init(
        mountSelector: ObjectId,
        options: Dict,
        params: Dict,
        widgetParams: Dict,
        funcs: Dict,
        defaults: Dict,
        onDone: (obj: any, iframe: any, rpc: any) => void,
        widgetId: Nullable<string | number>,
        iter: Nullable<number>
    ) {
    ...
    // pass story list
    openReader(
        payload: ClickOnStoryInternalWithDataPayload & { hasShare: boolean },
        sourceInfo: { listType: STORY_LIST_TYPE; feed: Option<string> }
    ) {
    ...
    setSoundOn(value: boolean) {
        RPC.setSoundOn(value);
    }

    destructor() {
        // destroy RPC
        // console.log("reader destructor");
    }
}
