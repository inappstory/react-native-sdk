import API, {ApiRequestConfig, ApiRequestConfigMethod} from "../api";
// import {ensureSessionId} from "~/src/stories_widget/models/Session";
import {debug} from "../../stories_widget/util/debug";
import {getSessionId} from "../../stories_widget/models/Session";
...
// TODO async client https://github.com/Microsoft/typed-rest-client/blob/master/lib/RestClient.ts
    protected static _fetch<A extends AbstractModel>(c: new () => A, resource: string, payLoad: { [index: string]: any }, method: ApiRequestConfigMethod = 'get'): Promise<A> {
     ...
     * @param method
     */
    public static update(resource: string, payLoad: IData, method: ApiRequestConfigMethod = 'post') {
    ...
    public static sendBeacon(path: string, data: IData, cb: () => void): void {
        if ("sendBeacon" in navigator) {
            ...
            const url = `${trimEndSlash(API.baseURL)}${trimStartSlash(path)}?access_token=${$storyManagerConfig.getState().apiKey}&session_id=${getSessionId()}`;
