...
import {CreateViewOptions} from "./web.h";
...
// import jsResource from 'js-url:~/dist/js/storiesList.js';
// const jsResourceGlobal = new URL("~/dist/js/storiesList.js", import.meta.url);
const jsResourceGlobal = new URL("~/src/exports/storiesList.js", import.meta.url);

// direct mount, not declarative
class WidgetStoriesList extends Widget implements IWidgetStoriesList {
    ...
    constructor(private appearanceManager: AppearanceManager, feedSlug?: string) {
        super();
    ...
    public get widgetEntrypoint(): string {
        return `
        <script type="text/javascript" src="${jsResourceGlobal}"></script>
    ...
    private createView(mountSelector: string, options: CreateViewOptions) {
        ...
        if (obj) {
            ...
            let width: number = options.width === 'auto' ? (obj.clientWidth || obj.offsetWidth || defaults.minWidth) | 0 : options.width;
